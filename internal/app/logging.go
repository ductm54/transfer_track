...
var (
	// SentryDSN is the CLI flag for Sentry DSN.
	SentryDSN = cli.StringFlag{ //nolint:gochecknoglobals
		Name:    "sentry-dsn",
		EnvVars: []string{"SENTRY_DSN"},
		Usage:   "dsn for sentry client",
	}
	// SentryLevel is the CLI flag for Sentry log level.
	...
	// CCLogAddress is the CLI flag for centralized logging address.
	...
	// CCLogName is the CLI flag for centralized logging name.
...
// SentryFlags returns the CLI flags for Sentry configuration.
...
func newLogger(c *cli.Context) (*zap.Logger, zap.AtomicLevel) {
	...
	if logAddr != "" && logName != "" {
		ccw := client.NewAsyncLogClient(logName, logAddr, func(err error) {
			_, _ = fmt.Fprintln(os.Stdout, "send log error", err)
		})
